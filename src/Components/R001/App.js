...
const Planet = (props) => {
  return (
    <div className="card">
      <div>
        <img className='planet' src={process.env.PUBLIC_URL+props.url} alt={props.name} />
      </div>
      <h2>{ props.name }</h2>
      <p>{ props.desc }</p>
...
export default function R001() {
  return (
    <div>
      <h3 className='center subtitle'>Project: {jsonData[0].projectName}</h3>
      <Container planetCards={planets} />
    </div>
  )
}
